...
const indentationMarkerConfig = state.Facet.define({
    combine(configs) {
...
function indentTheme(colorOptions) {
    ...
    return [theme, view.EditorView.editorAttributes.of({
            style: [
...
class IndentMarkersClass {
    ...
    generate(state$1) {
        ...
        const lines = getVisibleLines(this.view, state$1);
        const { hideFirstIndent, markerType, thickness, activeThickness } = state$1.facet(indentationMarkerConfig);
        const map = new IndentationMap(lines, state$1, this.unitWidth, markerType);
        for (const line of lines) {
            ...
            builder.add(line.from, line.from, view.Decoration.line({
...
function indentationMarkers(config = {}) {
    return [
        ...
        view.ViewPlugin.fromClass(IndentMarkersClass, {
